...
if (!sourcePath) throw new Error("Kullanım: node scripts/generate_iç hastalıkları hemşireliği_tezli_course_packages.mjs <ders-verileri.json>");
...
const program = data.programs.find((item) => fold(item.name) === "ic hastaliklari hemsireligi tezli yuksek lisans");
if (!program) throw new Error("İç Hastalıkları Hemşireliği Tezli Yüksek Lisans programı bulunamadı.");

const programOutcomes = [
  "İç hastalıkları hemşireliğindeki ileri düzey bilgileri uygular.",
  "Bireyin bakım gereksinimlerini bütüncül olarak analiz eder.",
  "Kanıta dayalı hemşirelik bakımını planlar, uygular ve değerlendirir.",
  "Klinik sorunlara bilimsel çözümler geliştirir.",
  "Bilimsel araştırma tasarlar, yürütür ve raporlar.",
  "Sağlık verilerini istatistiksel yöntemlerle analiz eder ve yorumlar.",
  "Disiplinler arası bakım süreçlerinde liderlik yapar ve iş birliği kurar.",
  "Sağlık ve bilişim teknolojilerini bakım ve araştırmada kullanır.",
  "Bilimsel, mesleki ve etik ilkeleri uygular.",
  "Bakım süreçlerini kalite ve hasta güvenliği açısından değerlendirir.",
  "Bilimsel sonuçları sunar ve mesleki gelişimine aktarır.",
];
...
const officialCourses = rawOfficial.split(/\r?\n/)
  .filter((line) => line.includes('department: "Hemşirelik ABD"') && line.includes('programName: "İç Hastalıkları Hemşireliği"') && line.includes('level: "Tezli Yüksek Lisans"'))
...
const domainFor = (name) => {
  ...
  if(/fizyopatoloji/u.test(n)) return {label:"ileri fizyopatoloji",core:[1,2,4],sdgs:["3","4","9"],terms:["hücresel hasar ve uyum","inflamasyon ve bağışıklık yanıtı","sıvı-elektrolit dengesi","organ sistemlerinin fizyopatolojisi","klinik bulguların mekanizmaları","tanısal göstergeler","komplikasyon riskleri","bakıma yansıyan fizyopatolojik veriler"]};
  if(/diyabet|kardiyo|noroloji|onkoloji|geriatri/u.test(n)) return {label:"sistem hastalıklarında ileri hemşirelik bakımı",core:[1,2,3],sdgs:["3","4","10"],terms:["hastalığın epidemiyolojisi ve fizyopatolojisi","risk ve klinik değerlendirme","kanıta dayalı hemşirelik bakımı","tedavi ve bakım izlemi","komplikasyonların önlenmesi","hasta ve aile eğitimi","disiplinler arası bakım","bakım sonuçlarının değerlendirilmesi"]};
  if(/palyatif|yogun bakim|semptom/u.test(n)) return {label:"karmaşık bakım ve semptom yönetimi",core:[2,3,10],sdgs:["3","4","10"],terms:["bütüncül hasta değerlendirmesi","semptom örüntüleri","kanıta dayalı semptom kontrolü","ileri bakım planlaması","hasta güvenliği","aile ve bakım veren desteği","etik karar verme","bakım kalitesinin değerlendirilmesi"]};
  if(/ilac|tamamlayici/u.test(n)) return {label:"ilaç güvenliği ve bütünleyici bakım",core:[3,4,9],sdgs:["3","4","12"],terms:["farmakolojik tedavi ilkeleri","akılcı ilaç kullanımı","ilaç etkileşimleri","ilaç güvenliği ve izlem","tamamlayıcı yaklaşım kanıtları","hasta eğitimi","etik ve mevzuat","klinik sonuçların değerlendirilmesi"]};
  if(/bakimin planlanmasi|ic hastaliklari/u.test(n)) return {label:"ileri iç hastalıkları hemşireliği",core:[1,2,3],sdgs:["3","4","10"],terms:["bütüncül sağlık değerlendirmesi","hemşirelik tanıları","kanıta dayalı bakım planı","klinik karar verme","bakım uygulamalarının yönetimi","hasta ve aile eğitimi","bakım koordinasyonu","kalite ve hasta güvenliği"]};
  ...
  return {label:"iç hastalıkları hemşireliği kuramı ve ileri uygulama",core:[1,3,11],sdgs:["3","4","10"],terms:["iç hastalıkları hemşireliğinin kavramsal temelleri","kuram ve model yaklaşımı","birey ve aile odaklı bakım","kanıta dayalı uygulama","klinik karar verme","mesleki roller","bakım kalitesi ve hasta güvenliği","güncel iç hastalıkları hemşireliği araştırmaları"]};
};
...
const outcomesFor = (name,domain) => {const subject=proseName(name); return [
  ...
  `${Sentence(subject)} kapsamında uygun iç hastalıkları hemşireliği yaklaşımını uygular.`,
...
const commonRawCodes=/^(DAN80[1-4]|İHH80[1-9]|İHH810)$/u;
const academicPackages=officialCourses.filter((o)=>!commonRawCodes.test(o.code)).map((official)=>{
  ...
  const purpose=detail(course,"dersin amaci")||`${sentenceStart(subject)} alanındaki kuramsal bilgileri, bilimsel kanıtları ve iç hastalıkları hemşireliği uygulamalarını lisansüstü düzeyde değerlendirme yetkinliği kazandırmak.`;
  ...
  const resourceTable=(course?.package?.tables||[]).find((item)=>fold(item.title)==="ders kaynaklari"); const resources=(resourceTable?.rows||[]).map((row)=>cells(row).filter(Boolean).join(": ")).join("; ")||"Dünya Sağlık Örgütü kronik hastalık ve hemşirelik bakım kaynakları; güncel klinik bakım rehberleri; konuya ilişkin güncel hakemli sağlık bilimleri literatürü.";
  ...
  return {code:official.code,name,department:"Hemşirelik ABD",programName:"İç Hastalıkları Hemşireliği",language:course?.language||"Türkçe",level:"Tezli Yüksek Lisans",teachingMode:source?.assignment?.teaching_method||"Yüz Yüze",theory,practice,credit:Number(source?.assignment?.local_credit??official.credit??theory),ects,prerequisites:"Yok",instructor:detail(course,"dersi verenler")||official.instructor||"Atama Bekliyor",purpose,content,methods,resources,sdgs:domain.sdgs,outcomes:outcomesFor(name,domain),weeklyTopics,assessments,workloads:workloadsFor(ects,theory,practice,assessments),contributionMatrix:matrixFor(domain),sourceUrl:course?.source_url,qualityChecks:checks(hasSource),publicQualityChecklist:false};
});
const commonSpecs=[
  {code:"DAN8XX",name:"DANIŞMANLIK",theory:0,practice:1,credit:0,ects:1,purpose:"Öğrencinin iç hastalıkları hemşireliği alanındaki akademik gelişimini ve tez araştırması sürecini bilimsel yöntem, birey merkezli bakım ve araştırma etiği bakımından yönlendirmek.",content:"Akademik planlama, iç hastalıkları hemşireliği literatürünün izlenmesi, araştırma probleminin geliştirilmesi, yöntem seçimi, etik uygunluk, ilerleme değerlendirmesi ve bilimsel raporlama süreçleri.",stages:["Akademik çalışma planının oluşturulması","İç Hastalıkları Hemşireliği araştırma alanının sınırlandırılması","İç hastalıkları hemşireliği ve kronik bakım literatürünün taranması","Araştırma probleminin netleştirilmesi","Etik ve mahremiyet gerekliliklerinin incelenmesi","Araştırma tasarımının değerlendirilmesi","Veri toplama planının izlenmesi","Araştırma kayıtlarının kontrolü","Bulguların ön değerlendirmesi","Analiz yaklaşımının gözden geçirilmesi","Bilimsel yazım planının oluşturulması","Kaynak ve atıf düzeninin denetlenmesi","Araştırma sınırlılıklarının değerlendirilmesi","Akademik gelişim ve ilerleme izlemesi","Sonraki dönem çalışma planının kararlaştırılması"]},
  {code:"İHH8XX",name:"UZMANLIK ALAN DERSİ",theory:4,practice:0,credit:0,ects:5,purpose:"Öğrencinin tez konusu ile ilişkili ileri iç hastalıkları hemşireliği literatürünü eleştirel değerlendirmesini ve araştırmasının kuramsal-yöntemsel altyapısını derinleştirmesini sağlamak.",content:"Tez alanına özgü iç hastalıkları hemşireliği literatürü, kronik ve karmaşık bakım bağlamı, araştırma problemi, kuramsal çerçeve, yöntem seçimi, veri yorumlama ve araştırma etiği.",stages:["Tez alanının bilimsel kapsamının belirlenmesi","İleri iç hastalıkları hemşireliği literatürünün sınıflandırılması","Bakım kuramlarının karşılaştırılması","Araştırma boşluğunun tanımlanması","Araştırma sorularının geliştirilmesi","Hipotezlerin bilimsel temellendirilmesi","Araştırma desenlerinin karşılaştırılması","Örnekleme yaklaşımının değerlendirilmesi","Ölçüm araçlarının incelenmesi","Veri kalite ölçütlerinin belirlenmesi","Analiz seçeneklerinin karşılaştırılması","Bulguların klinik yorum çerçevesi","Etik ve mahremiyet risklerinin değerlendirilmesi","Araştırma sınırlılıklarının tartışılması","Kuramsal ve yöntemsel çerçevenin bütünleştirilmesi"]},
  {code:"İHH806",name:"SEMİNER",theory:0,practice:0,credit:0,ects:6,purpose:"İç Hastalıkları Hemşireliği alanında bilimsel bir problemi literatüre dayalı inceleme, akademik metne dönüştürme ve bilimsel ortamda savunma becerisi kazandırmak.",content:"Literatür tarama, araştırma problemi, kanıta dayalı iç hastalıkları hemşireliği bilgisi, bilimsel metin, bulguların sentezi, akademik anlatım ve tartışma.",stages:["Seminer konusunun ve kapsamının belirlenmesi","Araştırma sorusunun geliştirilmesi","Literatür tarama stratejisinin kurulması","Kaynakların güvenilirlik bakımından değerlendirilmesi","Literatürün tematik sınıflandırılması","İç Hastalıkları Hemşireliği kanıtlarının karşılaştırılması","Seminer metninin iskeletinin oluşturulması","Yöntem ve bulguların sentezlenmesi","Tablo ve görsellerin bilimsel düzenlenmesi","Bilimsel tartışmanın yapılandırılması","Sonuç ve çıkarımların oluşturulması","Atıf ve kaynakça denetimi","Akademik anlatım tasarımının geliştirilmesi","Bilimsel anlatımın uygulanması","Geri bildirim doğrultusunda nihai düzenleme"]},
  {code:"İHH809",name:"BİLİMSEL ARAŞTIRMA YÖNTEMLERİ VE YAYIN ETİĞİ",theory:3,practice:0,credit:3,ects:6,purpose:"İç Hastalıkları Hemşireliği araştırmalarını bilimsel yöntem ve yayın etiği ilkelerine uygun tasarlama, yürütme, analiz etme ve raporlama yeterliliği kazandırmak.",content:"Araştırma problemi, sağlık araştırması desenleri, örnekleme, veri toplama, analiz yaklaşımı, bilimsel yazım, araştırma bütünlüğü, yayın etiği ve hassas grupların korunması.",stages:["Bilimsel bilgi ve iç hastalıkları hemşireliği araştırmasının özellikleri","Araştırma problemi ve hipotez geliştirme","Nicel araştırma desenleri","Nitel araştırma desenleri","Örnekleme ve çalışma grubu","Veri toplama araçları ve geçerlik","Sağlık verilerinde kalite güvencesi","Temel veri analizi yaklaşımları","Bulguların yorumlanması","Bilimsel makalenin yapısı","Kaynak kullanımı ve atıf ilkeleri","Araştırma bütünlüğü","Yayın etiği ihlallerinin değerlendirilmesi","Etik kurul ve hassas grupların korunması","Araştırma tasarımının bütüncül değerlendirilmesi"]},
  {code:"İHH81X",name:"TEZ ÇALIŞMASI",theory:0,practice:0,credit:0,ects:24,purpose:"Öğrencinin iç hastalıkları hemşireliği alanında bağımsız bir araştırmayı etik ilkelere uygun biçimde tasarlaması, yürütmesi, çözümlemesi ve bilimsel olarak savunmasını sağlamak.",content:"Özgün iç hastalıkları hemşireliği araştırma problemi, literatür sentezi, araştırma tasarımı, etik süreç, veri üretimi, kalite kontrolü, analiz, klinik yorumlama, tez yazımı ve savunma.",stages:["Araştırma probleminin kesinleştirilmesi","Literatür çerçevesinin güncellenmesi","Araştırma amaç ve hipotezlerinin yapılandırılması","Yöntem ve örnekleme planının kesinleştirilmesi","Etik kurul ve kurum izinlerinin tamamlanması","Veri toplama sürecinin planlanması","Katılımcı güvenliği ve veri kalite kontrolü","Veri toplama sürecinin izlenmesi","Verilerin düzenlenmesi ve doğrulanması","Analizlerin yürütülmesi","Bulguların iç hastalıkları hemşireliği bakımı açısından yorumlanması","Bulguların literatürle karşılaştırılması","Tez bölümlerinin bilimsel yazımı","Tez metninin bütünlük ve etik denetimi","Savunma sürecinin bilimsel ölçütlerle yapılandırılması"]},
];
const commonPackages=commonSpecs.map((spec)=>{const domain=domainFor(spec.name);const assessments=spec.code==="İHH806"?[{name:"Seminer Metni ve Sunumu",count:1,weight:100}]:spec.code==="İHH809"?[{name:"Ara Sınav",count:1,weight:40},{name:"Yarıyıl Sonu Sınavı",count:1,weight:60}]:[{name:"Başarılı / Başarısız",count:1,weight:100}];return {...spec,department:"Hemşirelik ABD",programName:"İç Hastalıkları Hemşireliği",language:"Türkçe",level:"Tezli Yüksek Lisans",teachingMode:spec.code==="İHH809"?"Yüz Yüze":"Bireysel Çalışma",prerequisites:"Yok",instructor:spec.code==="İHH809"?"Doç. Dr. DERYA TÜLÜCE":"Öğrencinin Danışmanı",methods:"Literatür incelemesi, akademik tartışma, bireysel araştırma ve danışmanlık görüşmesi.",resources:"Dünya Sağlık Örgütü kronik hastalık ve hemşirelik bakım kaynakları; güncel iç hastalıkları hemşireliği rehberleri; güncel hakemli klinik hemşirelik literatürü.",sdgs:domain.sdgs,outcomes:outcomesFor(spec.name,domain),weeklyTopics:spec.stages,assessments,workloads:workloadsFor(spec.ects,spec.theory,spec.practice,assessments),contributionMatrix:matrixFor(domain),qualityChecks:checks(true),publicQualityChecklist:false};});
writeFileSync(path.join(process.cwd(),"lib","data","icHastaliklariHemsireligiTezliCoursePackages.ts"),`// ${path.basename(sourcePath)} ve resmi LEE_DBP ders kataloğundan üretilmiştir; program profili ve PÇ kayıtları değiştirilmemiştir.\nimport type { CoursePackage } from "./coursePackages";\n\nexport const icHastaliklariHemsireligiTezliCoursePackages: CoursePackage[] = ${JSON.stringify(academicPackages,null,2)};\n`,`utf8`);
writeFileSync(path.join(process.cwd(),"lib","data","icHastaliklariHemsireligiCommonCoursePackages.ts"),`// İç Hastalıkları Hemşireliği Tezli Yüksek Lisans ortak/süreç dersleri alan bağlamına uyarlanmıştır.\nimport type { CoursePackage } from "./coursePackages";\n\nexport const icHastaliklariHemsireligiCommonCoursePackages: CoursePackage[] = ${JSON.stringify(commonPackages,null,2)};\n`,`utf8`);
