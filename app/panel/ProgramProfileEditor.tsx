...
export function ProgramProfileEditor({
 ...
 const[outcomes,setOutcomes]=useState(()=>getProgramProfile(programName,initialLevel)?.outcomes??initialOutcomes);
 const[programLevel,setProgramLevel]=useState(initialLevel);
 const[correctionOpen,setCorrectionOpen]=useState(false);
 const[correctionNote,setCorrectionNote]=useState("");
 const profile=useMemo(()=>getProgramProfile(programName,programLevel),[programName,programLevel]);
 const sections=profile?.sections??defaultSections.map(([title,text])=>({title,text}));
 const selectLevel=(level:string)=>{setProgramLevel(level);setOutcomes(getProgramProfile(programName,level)?.outcomes??initialOutcomes)};
 ...
 return <section className="program-editor"><div className="panel-intro"><div><h2>Program genel bilgileri</h2><p>{mode==="review"?"Program bilgilerini inceleyin; gerekiyorsa ABD/ASD başkanından düzeltme isteyin.":"ABD/ASD programının public Bologna profilinde yayımlanacak bilgileri düzenleyin."}</p><div className="program-level-strip">{availableLevels.map((level)=><button type="button" className={programLevel===level?"active":""} onClick={()=>selectLevel(level)} key={level}>{level.replace("Yüksek Lisans","YL")}</button>)}</div></div><span>{department}</span></div><form key={`${programName}-${programLevel}`} onSubmit={event=>{event.preventDefault();onSave()}}>
  <section className="program-summary-card"><h2>{programName} {programLevel==="Doktora"?"Doktora":"Yüksek Lisans"}</h2><div className="program-summary-grid"><label><span>Program adı</span><input defaultValue={profile?`${profile.programName} ${profile.level}`:`${programName} ${programLevel==="Doktora"?"Doktora":"Yüksek Lisans"}`}/></label><label><span>Program düzeyi</span><select value={programLevel} onChange={event=>selectLevel(event.target.value)}><option>Tezsiz Yüksek Lisans</option><option>Tezli Yüksek Lisans</option><option>Doktora</option></select></label><label><span>Kazanılan derece</span><input defaultValue={profile?.degree??`${programName} ${programLevel==="Doktora"?"Doktora":"Yüksek Lisans"} Derecesi`}/></label><label><span>Program yöneticisi</span><input defaultValue={profile?.manager??"Prof. Dr. Program Yöneticisi"}/></label><label className="wide"><span>Yeterlilik koşulları ve kuralları</span><input defaultValue={profile?.qualificationRules??(programLevel==="Doktora"?"4 yıl, 8 yarıyıl ve doktora yeterlilik/tez süreçleri":"2 yıl (120 AKTS), 4 yarıyıl, her yarıyılda 16–18 hafta")}/></label><label><span>Öğrenim dili</span><select defaultValue={profile?.language??"Türkçe"}><option>Türkçe</option><option>İngilizce</option><option>Türkçe / İngilizce</option></select></label></div></section>
  {sections.slice(0,2).map(({title,text})=><section className="program-content-card" key={title}><label>{title}</label><textarea defaultValue={text}/></section>)}
  ...
  <section className="program-content-card"><label>{sections[8].title}</label><textarea defaultValue={sections[8].text}/></section>
  {sections.slice(2,8).map(({title,text})=><section className="program-content-card" key={title}><label>{title}</label><textarea defaultValue={text}/></section>)}
  <ProgramTyycMatrix outcomeCount={outcomes.length} programLevel={programLevel} initialRows={profile?.tyycRows}/>
