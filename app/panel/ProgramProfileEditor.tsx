...
import { getProgramProfile, saveProgramProfile } from "../../lib/data/programProfiles";
...
export function ProgramProfileEditor({
 ...
 const persistProfile=(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const data=new FormData(event.currentTarget);const current=getProgramProfile(programName,programLevel);saveProgramProfile({programName,level:programLevel,degree:String(data.get("degree")??""),manager:String(data.get("manager")??""),language:String(data.get("language")??"Türkçe"),qualificationRules:String(data.get("qualificationRules")??""),sections:sections.map((section,index)=>({...section,text:String(data.get(`section-${index}`)??section.text)})),outcomes,tyycRows:current?.tyycRows??[]});onSave()};
 return <section className="program-editor"><div className="panel-intro"><div><h2>Program genel bilgileri</h2><p>{mode==="review"?"Program bilgilerini inceleyin; gerekiyorsa ABD/ASD başkanından düzeltme isteyin.":"ABD/ASD programının public Bologna profilinde yayımlanacak bilgileri düzenleyin."}</p><div className="program-level-strip">{availableLevels.map((level)=><button type="button" className={programLevel===level?"active":""} onClick={()=>selectLevel(level)} key={level}>{level.replace("Yüksek Lisans","YL")}</button>)}</div></div><span>{department}</span></div><form key={`${programName}-${programLevel}`} onSubmit={persistProfile}>
  <section className="program-summary-card"><h2>{programName} {programLevel==="Doktora"?"Doktora":"Yüksek Lisans"}</h2><div className="program-summary-grid"><label><span>Program adı</span><input readOnly defaultValue={profile?`${profile.programName} ${profile.level}`:`${programName} ${programLevel==="Doktora"?"Doktora":"Yüksek Lisans"}`}/></label><label><span>Program düzeyi</span><select value={programLevel} onChange={event=>selectLevel(event.target.value)}><option>Tezsiz Yüksek Lisans</option><option>Tezli Yüksek Lisans</option><option>Doktora</option></select></label><label><span>Kazanılan derece</span><input name="degree" defaultValue={profile?.degree??`${programName} ${programLevel==="Doktora"?"Doktora":"Yüksek Lisans"} Derecesi`}/></label><label><span>Program yöneticisi</span><input name="manager" defaultValue={profile?.manager??"Prof. Dr. Program Yöneticisi"}/></label><label className="wide"><span>Yeterlilik koşulları ve kuralları</span><input name="qualificationRules" defaultValue={profile?.qualificationRules??(programLevel==="Doktora"?"4 yıl, 8 yarıyıl ve doktora yeterlilik/tez süreçleri":"2 yıl (120 AKTS), 4 yarıyıl, her yarıyılda 16–18 hafta")}/></label><label><span>Öğrenim dili</span><select name="language" defaultValue={profile?.language??"Türkçe"}><option>Türkçe</option><option>İngilizce</option><option>Türkçe / İngilizce</option></select></label></div></section>
