...
function rows(term,index){
  ...
  if(official.length){
    return official.map((course)=>`<tr><td><b>${course.code}</b></td><td>${repairText(course.name)}</td><td><span class="course-type ${repairText(course.type)==="Zorunlu"?"required":"elective"}">${repairText(course.type)}</span></td><td>${course.theory}</td><td>${course.practice}</td><td><b>${course.ects}</b></td><td><a class="local-table-action primary" href="${courseUrl(course)}">▣ <span>Görüntüle</span></a></td><td><a class="local-table-action" href="${coursePdfUrl(course)}" target="_blank" rel="noreferrer">▧ <span>Yazdır</span></a></td></tr>`).join("");
  }
