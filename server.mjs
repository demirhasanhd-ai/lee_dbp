...
const aileTezsizProjectCodes = new Set(["ADE701", "ADE702"]);
const aileTezsizCourseNames = new Map([
  ...
  ["ADE709", "Aile İçi İlişkiler ve İletişim"], ["ADE710", "Ailede Cinsellik ve Cinsel Sorunlar"], ["ADE711", "Aile Olmak"],
  ["ADE712", "Ailede Değer Eğitimi"], ["ADE713", "Aile Sosyolojisi"], ["ADE714", "Ailede Krize Müdahale"],
  ["ADE715", "Ailede Gelişim Psikolojisi"], ["ADE716", "Bireysel Danışma Kuramları"], ["ADE717", "Ailede Ruh Sağlığı"],
  ["ADE718", "Ailede Özel Sorunlar (Boşanma, Aldatma, Ölüm ve Yas)"], ["ADE719", "Çocuklarda Davranış Bozukluğu ve Aile"],
...
const ybsTezsizProjectCodes = new Set(["YBS701", "YBS702"]);
const ybsTezsizResearchCodes = new Set(["YBS703", "YBS704"]);
const yonetimOrganizasyonTezsizProjectCodes = new Set(["YON701", "YON702"]);
const yonetimOrganizasyonTezsizResearchCodes = new Set(["YON703", "BES704"]);
...
const isletmeDrAdvisoryCodes = new Set(["DAN901", "DAN902", "DAN903", "DAN904", "DAN905", "DAN906", "DAN907", "DAN908"]);
const isletmeDrSpecializationCodes = new Set(["ISL901", "ISL902", "ISL903", "ISL904", "ISL905", "ISL906", "ISL907", "ISL908"]);
const isletmeDrSeminarCodes = new Set(["ISL909", "ISL910"]);
const isletmeDrQualifyingCodes = new Set(["ISL917", "ISL918"]);
const isletmeDrThesisCodes = new Set(["ISL911", "ISL912", "ISL913", "ISL914", "ISL915", "ISL916"]);
const kimyaDrAdvisoryCodes = new Set(["DAN901", "DAN902", "DAN903", "DAN904", "DAN905", "DAN906", "DAN907", "DAN908"]);
const kimyaDrSpecializationCodes = new Set(["KİM901", "KİM902", "KİM903", "KİM904", "KİM905", "KİM906", "KİM907", "KİM908"]);
const kimyaDrSeminarCodes = new Set(["KİM909", "KİM910"]);
...
const turkDiliEdebiyatiDrAdvisoryCodes = new Set(["DAN901", "DAN902"]);
const turkDiliEdebiyatiDrSpecializationCodes = new Set(["TDE901", "TDE902", "TDE903", "TDE904", "TDE905", "TDE906", "TDE907", "TDE908"]);
const turkDiliEdebiyatiDrResearchCodes = new Set(["TDE909", "TDE1012"]);
const turkDiliEdebiyatiDrSeminarCodes = new Set(["TDE910"]);
const turkDiliEdebiyatiDrQualifyingCodes = new Set(["TDE917", "TDE918"]);
const turkDiliEdebiyatiDrThesisCodes = new Set(["TDE912", "TDE913", "TDE914", "TDE915", "TDE916"]);
...
function canonicalCourseCode(code = "") {
  ...
  if (ybsTezsizProjectCodes.has(normalizedCode)) return "YBS7XX";
  if (ybsTezsizResearchCodes.has(normalizedCode)) return "YBS703";
  if (yonetimOrganizasyonTezsizProjectCodes.has(normalizedCode)) return "YON7XX";
  if (yonetimOrganizasyonTezsizResearchCodes.has(normalizedCode)) return "YON703";
  ...
  if (isletmeDrSpecializationCodes.has(normalizedCode)) return "ISL9XX";
  if (isletmeDrSeminarCodes.has(normalizedCode)) return "ISL909";
  if (isletmeDrQualifyingCodes.has(normalizedCode)) return "ISL917";
  if (isletmeDrThesisCodes.has(normalizedCode)) return "ISL91X";
  if (kimyaDrSpecializationCodes.has(normalizedCode)) return "KİM9XX";
  if (kimyaDrSeminarCodes.has(normalizedCode)) return "KİM909";
  ...
  if (turkDiliEdebiyatiDrSpecializationCodes.has(normalizedCode)) return "TDE9XX";
  if (turkDiliEdebiyatiDrResearchCodes.has(normalizedCode)) return "TDE909";
  if (turkDiliEdebiyatiDrSeminarCodes.has(normalizedCode)) return "TDE910";
  if (turkDiliEdebiyatiDrQualifyingCodes.has(normalizedCode)) return "TDE917";
  if (turkDiliEdebiyatiDrThesisCodes.has(normalizedCode)) return "TDE91X";
...
function courseCodeCandidates(code = "") {
  ...
  if (canonical === "TDE9XX") for (const alias of turkDiliEdebiyatiDrSpecializationCodes) candidates.add(alias);
  if (canonical === "TDE909") for (const alias of turkDiliEdebiyatiDrResearchCodes) candidates.add(alias);
  if (canonical === "TDE910") for (const alias of turkDiliEdebiyatiDrSeminarCodes) candidates.add(alias);
  if (canonical === "TDE917") for (const alias of turkDiliEdebiyatiDrQualifyingCodes) candidates.add(alias);
  if (canonical === "TDE91X") for (const alias of turkDiliEdebiyatiDrThesisCodes) candidates.add(alias);
  ...
  if (canonical === "YBS7XX") for (const alias of ybsTezsizProjectCodes) candidates.add(alias);
  if (canonical === "YBS703") for (const alias of ybsTezsizResearchCodes) candidates.add(alias);
  if (canonical === "YON7XX") for (const alias of yonetimOrganizasyonTezsizProjectCodes) candidates.add(alias);
  if (canonical === "YON703") for (const alias of yonetimOrganizasyonTezsizResearchCodes) candidates.add(alias);
  ...
  if (canonical === "DAN9XX") for (const alias of isletmeDrAdvisoryCodes) candidates.add(alias);
  if (canonical === "ISL9XX") for (const alias of isletmeDrSpecializationCodes) candidates.add(alias);
  if (canonical === "ISL909") for (const alias of isletmeDrSeminarCodes) candidates.add(alias);
  if (canonical === "ISL917") for (const alias of isletmeDrQualifyingCodes) candidates.add(alias);
  if (canonical === "ISL91X") for (const alias of isletmeDrThesisCodes) candidates.add(alias);
  if (canonical === "DAN9XX") for (const alias of kimyaDrAdvisoryCodes) candidates.add(alias);
  if (canonical === "KİM9XX") for (const alias of kimyaDrSpecializationCodes) candidates.add(alias);
  if (canonical === "KİM909") for (const alias of kimyaDrSeminarCodes) candidates.add(alias);
...
function normalizeEnerjiTezsizCourse(course = {}) {
  const applies = levelKey(course.level) === "tezsiz yl" &&
    normalizeScope(course.department || "") === normalizeScope("Enerji Sistemleri Mühendisliği ABD") &&
    normalizeScope(course.programName || course.program_name || "") === normalizeScope("Enerji Sistemleri Mühendisliği");
  if (!applies) return course;
  const code = repairText(course.code || "").trim().toLocaleUpperCase("tr-TR");
  if (enerjiTezsizProjectCodes.has(code)) {
    return code === "EPY701"
      ? { ...course, code:"EPY7XX", name:"BİTİRME PROJESİ", ects:30, instructor:"Öğrencinin Danışmanı" }
      : null;
  }
  if (code === "EPY704" || code === "EPY512") return null;
  const completeNames = new Map([
    ["EPY703", "BİLİMSEL ARAŞTIRMA YÖNTEMLERİ VE YAYIN ETİĞİ"],
    ["EPY706", "ENERJİ TEKNOLOJİLERİ"],
    ["EPY710", "ENERJİ VERİMLİLİĞİ, FİZİBİLİTE ETÜDÜ VE VAP UYGULAMALARI"],
    ["EPY718", "KONVANSİYONEL ENERJİ KAYNAKLARI"],
  ]);
  const completeName = completeNames.get(code);
  return completeName ? { ...course, name:completeName } : course;
}

function normalizeGastronomiTezsizCourse(course = {}) {
  const applies = levelKey(course.level) === "tezsiz yl" &&
    normalizeScope(course.department || "") === normalizeScope("Gastronomi ve Mutfak Sanatları ABD") &&
    normalizeScope(course.programName || course.program_name || "") === normalizeScope("Gastronomi ve Mutfak Sanatları");
  if (!applies) return course;
  const code = repairText(course.code || "").trim().toLocaleUpperCase("tr-TR");
  if (gastronomiTezsizProjectCodes.has(code)) {
    return code === "GMS701"
      ? { ...course, code:"GMS7XX", name:"BİTİRME PROJESİ", ects:30, instructor:"Öğrencinin Danışmanı" }
      : null;
  }
  if (code === "GMS704") return null;
  const completeNames = new Map([
...
function normalizeGidaTeknolojisiTezsizCourse(course = {}) {
  const applies = levelKey(course.level) === "tezsiz yl" &&
    normalizeScope(course.department || "") === normalizeScope("Gıda Teknolojisi ABD") &&
    normalizeScope(course.programName || course.program_name || "") === normalizeScope("Gıda Teknolojisi");
  if (!applies) return course;
  const code = repairText(course.code || "").trim().toLocaleUpperCase("tr-TR");
  ...
  if (code === "GTB703") return { ...course, name:"BİLİMSEL ARAŞTIRMA VE YAYIN ETİĞİ" };
  return course;
}
...
function normalizeYbsTezsizCourse(course = {}) {
  const applies = levelKey(course.level) === "tezsiz yl" &&
    normalizeScope(course.department || "") === normalizeScope("Yönetim Bilişim Sistemleri ABD") &&
    normalizeScope(course.programName || course.program_name || "") === normalizeScope("Yönetim Bilişim Sistemleri");
  if (!applies) return course;
  const code = repairText(course.code || "").trim().toLocaleUpperCase("tr-TR");
  const name = repairText(course.name || "");
  if (/DANIŞMANLIK|UZMANLIK ALAN/iu.test(name) || /^DAN7/u.test(code)) return null;
  if (ybsTezsizProjectCodes.has(code)) return code === "YBS701" ? { ...course, code:"YBS7XX", name:"BİTİRME PROJESİ", ects:30, term:"Güz ve Bahar", instructor:"Öğrencinin Danışmanı" } : null;
  if (ybsTezsizResearchCodes.has(code)) return code === "YBS703" ? { ...course, code:"YBS703", name:"BİLİMSEL ARAŞTIRMA YÖNTEMLERİ VE YAYIN ETİĞİ", ects:6, term:"Güz ve Bahar" } : null;
  return course;
}

function normalizeYonetimOrganizasyonTezsizCourse(course = {}) {
  const applies = levelKey(course.level) === "tezsiz yl" &&
    normalizeScope(course.department || "") === normalizeScope("Yönetim Organizasyon") &&
    normalizeScope(course.programName || course.program_name || "") === normalizeScope("Yönetim Organizasyon");
  if (!applies) return course;
  const code = repairText(course.code || "").trim().toLocaleUpperCase("tr-TR");
  const name = repairText(course.name || "");
  if (/DANIŞMANLIK|UZMANLIK ALAN/iu.test(name) || /^DAN7/u.test(code)) return null;
  if (yonetimOrganizasyonTezsizProjectCodes.has(code)) return code === "YON701" ? { ...course, code:"YON7XX", name:"BİTİRME PROJESİ", ects:30, term:"Güz ve Bahar", instructor:"Öğrencinin Danışmanı" } : null;
  if (yonetimOrganizasyonTezsizResearchCodes.has(code)) return code === "YON703" ? { ...course, code:"YON703", name:"BİLİMSEL ARAŞTIRMA YÖNTEMLERİ VE YAYIN ETİĞİ", ects:6, term:"Güz ve Bahar" } : null;
  return course;
}
...
function normalizeAileTezsizCourse(course = {}) {
  ...
  if (!applies) return course;
  const code = repairText(course.code || "").trim().toLocaleUpperCase("tr-TR");
  if (!aileTezsizProjectCodes.has(code)) return aileTezsizCourseNames.has(code) ? { ...course, name: aileTezsizCourseNames.get(code) } : course;
  if (code !== "ADE701") return null;
  return { ...course, code: "ADE7XX", name: "BİTİRME PROJESİ", ects: 30, instructor: "Öğrencinin Danışmanı" };
}
...
function normalizeIsletmeDoktoraCourse(course = {}) {
  const applies = levelKey(course.level) === "doktora" && normalizeScope(course.department || "") === normalizeScope("İşletme") && normalizeScope(course.programName || course.program_name || "") === normalizeScope("İşletme");
  if (!applies) return course;
  const code=repairText(course.code||"").trim().toLocaleUpperCase("tr-TR");
  if(isletmeDrAdvisoryCodes.has(code))return code==="DAN901"?{...course,code:"DAN9XX",name:"DANIŞMANLIK",ects:1,instructor:"Öğrencinin Danışmanı"}:null;
  if(isletmeDrSpecializationCodes.has(code))return code==="ISL901"?{...course,code:"ISL9XX",name:"UZMANLIK ALAN DERSİ",ects:5,instructor:"Öğrencinin Danışmanı"}:null;
  if(isletmeDrSeminarCodes.has(code))return code==="ISL909"?{...course,code:"ISL909",name:"SEMİNER",ects:6,instructor:"Öğrencinin Danışmanı"}:null;
  if(isletmeDrQualifyingCodes.has(code))return code==="ISL917"?{...course,code:"ISL917",name:"DOKTORA YETERLİK",ects:24,instructor:"Öğrencinin Danışmanı"}:null;
  if(isletmeDrThesisCodes.has(code))return code==="ISL912"?{...course,code:"ISL91X",name:"TEZ ÇALIŞMASI",ects:24,instructor:"Öğrencinin Danışmanı"}:null;
  return course;
}

function normalizeKimyaDoktoraCourse(course = {}) {
  const applies = levelKey(course.level) === "doktora" && normalizeScope(course.department || "") === normalizeScope("Kimya ABD") && normalizeScope(course.programName || course.program_name || "") === normalizeScope("Kimya");
  if (!applies) return course;
  const code=repairText(course.code||"").trim().toLocaleUpperCase("tr-TR");
  if(kimyaDrAdvisoryCodes.has(code))return code==="DAN901"?{...course,code:"DAN9XX",name:"DANIŞMANLIK",ects:1,instructor:"Öğrencinin Danışmanı"}:null;
  if(kimyaDrSpecializationCodes.has(code))return code==="KİM901"?{...course,code:"KİM9XX",name:"UZMANLIK ALAN DERSİ",ects:5,instructor:"Öğrencinin Danışmanı"}:null;
  if(kimyaDrSeminarCodes.has(code))return code==="KİM909"?{...course,code:"KİM909",name:"SEMİNER",ects:6,instructor:"Öğrencinin Danışmanı"}:null;
  if(kimyaDrResearchCodes.has(code))return code==="KİM931"?{...course,code:"KİM931",name:"BİLİMSEL ARAŞTIRMA YÖNTEMLERİ VE YAYIN ETİĞİ",ects:6}:null;
  if(kimyaDrQualifyingCodes.has(code))return code==="KİM917"?{...course,code:"KİM917",name:"DOKTORA YETERLİK",ects:24,instructor:"Öğrencinin Danışmanı"}:null;
  if(kimyaDrThesisCodes.has(code))return code==="KİM912"?{...course,code:"KİM91X",name:"TEZ ÇALIŞMASI",ects:24,instructor:"Öğrencinin Danışmanı"}:null;
  return course;
}
...
function normalizeMakineDoktoraCourse(course = {}) {
  ...
  if(makineDrResearchCodes.has(code))return code==="MMB951"?{...course,code:"MMB951",name:"Bilimsel Araştırma Yöntemleri ve Yayın Etiği",term:"Güz ve Bahar",ects:6}:null;
...
function normalizeTurkDiliEdebiyatiDoktoraCourse(course = {}) {
  const applies = levelKey(course.level) === "doktora" && normalizeScope(course.department || "") === normalizeScope("Türk Dili ve Edebiyatı ABD") && normalizeScope(course.programName || course.program_name || "") === normalizeScope("Türk Dili ve Edebiyatı");
  if (!applies) return course;
  const code=repairText(course.code||"").trim().toLocaleUpperCase("tr-TR");
  if(turkDiliEdebiyatiDrAdvisoryCodes.has(code))return code==="DAN901"?{...course,code:"DAN9XX",name:"DANIŞMANLIK",ects:1,instructor:"Öğrencinin Danışmanı"}:null;
  if(turkDiliEdebiyatiDrSpecializationCodes.has(code))return code==="TDE901"?{...course,code:"TDE9XX",name:"UZMANLIK ALAN DERSİ",ects:5,instructor:"Öğrencinin Danışmanı"}:null;
  if(turkDiliEdebiyatiDrResearchCodes.has(code))return code==="TDE909"?{...course,code:"TDE909",name:"BİLİMSEL ARAŞTIRMA YÖNTEMLERİ VE YAYIN ETİĞİ",ects:6}:null;
  if(turkDiliEdebiyatiDrSeminarCodes.has(code))return code==="TDE910"?{...course,code:"TDE910",name:"SEMİNER",ects:6,instructor:"Öğrencinin Danışmanı"}:null;
  if(turkDiliEdebiyatiDrQualifyingCodes.has(code))return code==="TDE917"?{...course,code:"TDE917",name:"DOKTORA YETERLİK",ects:24,instructor:"Öğrencinin Danışmanı"}:null;
  if(turkDiliEdebiyatiDrThesisCodes.has(code))return code==="TDE912"?{...course,code:"TDE91X",name:"TEZ ÇALIŞMASI",ects:24,instructor:"Öğrencinin Danışmanı"}:null;
  return course;
}
...
function normalizeSeedCourse(course = {}) {
  ...
  const aileTezsizCourse = normalizeAileTezsizCourse(repaired);
  if (!aileTezsizCourse) return null;
  if (aileTezsizCourse !== repaired) return aileTezsizCourse;
  ...
  const enerjiTezsizCourse = normalizeEnerjiTezsizCourse(repaired);
  if (!enerjiTezsizCourse) return null;
  if (enerjiTezsizCourse !== repaired) return enerjiTezsizCourse;
  const gastronomiTezsizCourse = normalizeGastronomiTezsizCourse(repaired);
  if (!gastronomiTezsizCourse) return null;
  if (gastronomiTezsizCourse !== repaired) return gastronomiTezsizCourse;
  ...
  const ybsTezsizCourse = normalizeYbsTezsizCourse(repaired);
  if (!ybsTezsizCourse) return null;
  if (ybsTezsizCourse !== repaired) return ybsTezsizCourse;
  const yonetimOrganizasyonTezsizCourse = normalizeYonetimOrganizasyonTezsizCourse(repaired);
  if (!yonetimOrganizasyonTezsizCourse) return null;
  if (yonetimOrganizasyonTezsizCourse !== repaired) return yonetimOrganizasyonTezsizCourse;
  ...
  const isletmeDoktoraCourse = normalizeIsletmeDoktoraCourse(repaired);
  if (!isletmeDoktoraCourse) return null;
  if (isletmeDoktoraCourse !== repaired) return isletmeDoktoraCourse;
  ...
  const turkDiliEdebiyatiDoktoraCourse = normalizeTurkDiliEdebiyatiDoktoraCourse(repaired);
  if (!turkDiliEdebiyatiDoktoraCourse) return null;
  if (turkDiliEdebiyatiDoktoraCourse !== repaired) return turkDiliEdebiyatiDoktoraCourse;
...
function isDepartmentPoolCourseRecord(course = {}) {
  ...
  if (["BES7XX", "BEF7XX", "BİO7XX", "İKT7XX", "EPY7XX", "GMS7XX", "GTB7XX", "YBS7XX", "YON7XX"].includes(repairText(course.code || "").toLocaleUpperCase("tr-TR"))) return true;
...
function canEditCoursePackage(session, body, rows) {
  ...
  if (session.role === "abd_asd_baskani") {
    ...
    trustedMergedPoolCodes.add("YBS7XX");
    trustedMergedPoolCodes.add("YON7XX");
...
async function ensureDb() {
  ...
  migrateEnerjiTezsizPackagesFromSeed();
  migrateGastronomiTezsizPackagesFromSeed();
  migrateGidaTeknolojisiTezsizPackagesFromSeed();
  ...
  migrateTurkDiliEdebiyatiDoktoraPackagesFromSeed();
...
function packageSeedCourseName(coursePackage) {
  const knownNames = {
    ...
    TDE9XX: "UZMANLIK ALAN DERSİ",
    TDE909: "BİLİMSEL ARAŞTIRMA YÖNTEMLERİ VE YAYIN ETİĞİ",
    TDE910: "SEMİNER",
    TDE917: "DOKTORA YETERLİK",
    TDE91X: "TEZ ÇALIŞMASI",
  ...
  try {
    for (const course of officialCourses) {
      ...
      const nextInstructor =
        seededInstructor && normalizeInstructorScope(seededInstructor) !== normalizeInstructorScope("Atama bekliyor")
          ? seededInstructor
          : sanitizeInstructorName(existing.instructor || seededInstructor || "");
      updateCourse.run(
...
function migrateEnerjiTezsizPackagesFromSeed() {
  ...
  const packages = readCoursePackageSeeds().filter((item) =>
    normalizeScope(item.department || "") === normalizeScope("Enerji Sistemleri Mühendisliği ABD") &&
    normalizeScope(item.programName || "") === normalizeScope("Enerji Sistemleri Mühendisliği") &&
  ...
  try {
    for (const packageSeed of packages) {
      const rows = courseRowsForIdentity({ department:packageSeed.department, programName:packageSeed.programName, level:packageSeed.level, code:packageSeed.code });
      for (const course of rows) {
        const courseName = packageSeed.name || course.name;
        update.run(courseName, Number(packageSeed.credit || 0), Number(packageSeed.ects || 0), Number(packageSeed.theory || 0), Number(packageSeed.practice || 0), JSON.stringify(storedPackageFromSeed(packageSeed, { ...course, name:courseName, programName:course.program_name })), now, course.id);
        changed += 1;
      }
    }
    db.prepare("INSERT OR REPLACE INTO metadata(key, value) VALUES (?, ?)").run(metadataKey, revision);
    audit("course.package.migrate", "system", { scope:"Enerji Sistemleri Mühendisliği Tezsiz YL", revision, changed });
    db.exec("COMMIT");
  } catch (error) {
    db.exec("ROLLBACK");
    throw error;
...
function migrateGastronomiTezsizPackagesFromSeed() {
  const revision = "2026-08-25-gastronomi-tezsiz-v1";
  const metadataKey = "gastronomi_tezsiz_packages_revision";
  if (db.prepare("SELECT value FROM metadata WHERE key = ?").get(metadataKey)?.value === revision) return;
  const packages = readCoursePackageSeeds().filter((item) =>
    normalizeScope(item.department || "") === normalizeScope("Gastronomi ve Mutfak Sanatları ABD") &&
    normalizeScope(item.programName || "") === normalizeScope("Gastronomi ve Mutfak Sanatları") &&
    levelKey(item.level || "") === "tezsiz yl"
  );
  const now = new Date().toISOString();
  const update = db.prepare(`UPDATE courses SET name = ?, credit = ?, ects = ?, theory = ?, practice = ?, status = 'Public', package_json = ?, updated_at = ? WHERE id = ?`);
  ...
  try {
    ...
    db.prepare("INSERT OR REPLACE INTO metadata(key, value) VALUES (?, ?)").run(metadataKey, revision);
    audit("course.package.migrate", "system", { scope:"Gastronomi ve Mutfak Sanatları Tezsiz YL", revision, changed });
    db.exec("COMMIT");
  } catch (error) {
    db.exec("ROLLBACK");
    throw error;
...
function migrateGidaTeknolojisiTezsizPackagesFromSeed() {
  const revision = "2026-08-25-gida-teknolojisi-tezsiz-v1";
  const metadataKey = "gida_teknolojisi_tezsiz_packages_revision";
  if (db.prepare("SELECT value FROM metadata WHERE key = ?").get(metadataKey)?.value === revision) return;
  const packages = readCoursePackageSeeds().filter((item) =>
    normalizeScope(item.department || "") === normalizeScope("Gıda Teknolojisi ABD") &&
    normalizeScope(item.programName || "") === normalizeScope("Gıda Teknolojisi") &&
    levelKey(item.level || "") === "tezsiz yl"
  );
  const now = new Date().toISOString();
  const update = db.prepare(`UPDATE courses SET name = ?, credit = ?, ects = ?, theory = ?, practice = ?, status = 'Public', package_json = ?, updated_at = ? WHERE id = ?`);
  ...
  try {
    ...
    db.prepare("INSERT OR REPLACE INTO metadata(key, value) VALUES (?, ?)").run(metadataKey, revision);
    audit("course.package.migrate", "system", { scope:"Gıda Teknolojisi Tezsiz YL", revision, changed });
    db.exec("COMMIT");
  } catch (error) {
    db.exec("ROLLBACK");
    throw error;
...
function migrateIsletmeDoktoraPackagesFromSeed() {
  const revision="2026-08-24-isletme-doktora-v1";if(db.prepare("SELECT value FROM metadata WHERE key = ?").get("isletme_doktora_packages_revision")?.value===revision)return;
  const packages=readCoursePackageSeeds().filter((x)=>normalizeScope(x.department||"")===normalizeScope("İşletme")&&normalizeScope(x.programName||"")===normalizeScope("İşletme")&&levelKey(x.level||"")==="doktora");const now=new Date().toISOString();const update=db.prepare(`UPDATE courses SET name = ?, credit = ?, ects = ?, theory = ?, practice = ?, status = 'Public', package_json = ?, updated_at = ? WHERE id = ?`);let changed=0;db.exec("BEGIN");try{for(const p of packages){const rows=courseRowsForIdentity({department:p.department,programName:p.programName,level:p.level,code:p.code});for(const c of rows){update.run(p.name||c.name,Number(p.credit||0),Number(p.ects||0),Number(p.theory||0),Number(p.practice||0),JSON.stringify(storedPackageFromSeed(p,{...c,programName:c.program_name})),now,c.id);changed+=1}}db.prepare("INSERT OR REPLACE INTO metadata(key, value) VALUES (?, ?)").run("isletme_doktora_packages_revision",revision);audit("course.package.migrate","system",{scope:"İşletme Doktora",revision,changed});db.exec("COMMIT")}catch(error){db.exec("ROLLBACK");throw error}
}
...
function migrateMakineMuhendisligiDoktoraPackagesFromSeed() {
  const revision="2026-09-01-makine-muhendisligi-doktora-v6-json-complete";
  if(db.prepare("SELECT value FROM metadata WHERE key = ?").get("makine_muhendisligi_doktora_packages_revision")?.value===revision)return;
  const packages=readCoursePackageSeeds().filter((x)=>normalizeScope(x.department||"")===normalizeScope("Makine Mühendisliği ABD")&&normalizeScope(x.programName||"")===normalizeScope("Makine Mühendisliği")&&levelKey(x.level||"")==="doktora");
  const now=new Date().toISOString();
  const update=db.prepare(`UPDATE courses SET name = ?, credit = ?, ects = ?, theory = ?, practice = ?, status = 'Public', package_json = ?, updated_at = ? WHERE id = ?`);
  const removeObsolete=db.prepare(`DELETE FROM courses WHERE department = ? AND program_name = ? AND level = ? AND code IN ('PFE901','PFE942')`);
  let changed=0;
  let removed=0;
  db.exec("BEGIN");
  try{
    removed=Number(removeObsolete.run("Makine Mühendisliği ABD","Makine Mühendisliği","Doktora").changes||0);
    for(const p of packages){
      const rows=courseRowsForIdentity({department:p.department,programName:p.programName,level:p.level,code:p.code});
      for(const c of rows){
        const courseName=p.name||c.name;
        update.run(courseName,Number(p.credit||0),Number(p.ects||0),Number(p.theory||0),Number(p.practice||0),JSON.stringify(storedPackageFromSeed(p,{...c,name:courseName,programName:c.program_name})),now,c.id);
        changed+=1;
      }
    }
    db.prepare("INSERT OR REPLACE INTO metadata(key, value) VALUES (?, ?)").run("makine_muhendisligi_doktora_packages_revision",revision);
    audit("course.package.migrate","system",{scope:"Makine Mühendisliği Doktora",revision,changed,removed});
    db.exec("COMMIT");
  }catch(error){
    db.exec("ROLLBACK");
    throw error;
...
function migrateTurkDiliEdebiyatiDoktoraPackagesFromSeed() {
  const revision="2026-08-24-turk-dili-edebiyati-doktora-v1";if(db.prepare("SELECT value FROM metadata WHERE key = ?").get("turk_dili_edebiyati_doktora_packages_revision")?.value===revision)return;
  const packages=readCoursePackageSeeds().filter((x)=>normalizeScope(x.department||"")===normalizeScope("Türk Dili ve Edebiyatı ABD")&&normalizeScope(x.programName||"")===normalizeScope("Türk Dili ve Edebiyatı")&&levelKey(x.level||"")==="doktora");const now=new Date().toISOString();const update=db.prepare(`UPDATE courses SET name = ?, credit = ?, ects = ?, theory = ?, practice = ?, status = 'Public', package_json = ?, updated_at = ? WHERE id = ?`);let changed=0;db.exec("BEGIN");try{for(const p of packages){const rows=courseRowsForIdentity({department:p.department,programName:p.programName,level:p.level,code:p.code});for(const c of rows){const courseName=p.name||c.name;update.run(courseName,Number(p.credit||0),Number(p.ects||0),Number(p.theory||0),Number(p.practice||0),JSON.stringify(storedPackageFromSeed(p,{...c,name:courseName,programName:c.program_name})),now,c.id);changed+=1}}db.prepare("INSERT OR REPLACE INTO metadata(key, value) VALUES (?, ?)").run("turk_dili_edebiyati_doktora_packages_revision",revision);audit("course.package.migrate","system",{scope:"Türk Dili ve Edebiyatı Doktora",revision,changed});db.exec("COMMIT")}catch(error){db.exec("ROLLBACK");throw error}
}
...
function migrateYonetimOrganizasyonTezsizPackagesFromSeed() {
  const revision="2026-09-01-yonetim-organizasyon-tezsiz-v1";
  if(db.prepare("SELECT value FROM metadata WHERE key = ?").get("yonetim_organizasyon_tezsiz_packages_revision")?.value===revision)return;
  const packages=readCoursePackageSeeds().filter((x)=>normalizeScope(x.department||"")===normalizeScope("Yönetim Organizasyon")&&normalizeScope(x.programName||"")===normalizeScope("Yönetim Organizasyon")&&levelKey(x.level||"")==="tezsiz yl");
  const now=new Date().toISOString();
  const update=db.prepare(`UPDATE courses SET name = ?, credit = ?, ects = ?, theory = ?, practice = ?, term = ?, instructor = ?, status = 'Public', package_json = ?, updated_at = ? WHERE id = ?`);
  let changed=0;
  db.exec("BEGIN");
  try{
    for(const p of packages){
      const rows=courseRowsForIdentity({department:p.department,programName:p.programName,level:p.level,code:p.code});
      for(const c of rows){
        update.run(p.name||c.name,Number(p.credit||0),Number(p.ects||0),Number(p.theory||0),Number(p.practice||0),p.code==="YON7XX"||p.code==="YON703"?"Güz ve Bahar":c.term,p.instructor||c.instructor||"",JSON.stringify(storedPackageFromSeed(p,{...c,programName:c.program_name})),now,c.id);
        changed+=1;
      }
    }
    db.prepare("INSERT OR REPLACE INTO metadata(key, value) VALUES (?, ?)").run("yonetim_organizasyon_tezsiz_packages_revision",revision);
    audit("course.package.migrate","system",{scope:"Yönetim Organizasyon Tezsiz YL",revision,changed});
    db.exec("COMMIT");
